...
router.get("/:name/rewards", function(req, res, next) {
  const query =
    "SELECT * FROM rewards WHERE project_name = " +
    "'" +
    req.params.name.split("_").join(" ") +
    "'";
  pool.query(query, (error, data) => {
...
router.get("/backedRewards/:name/:email", function(req, res, next) {
  const query =
    "SELECT reward_name FROM backingfunds WHERE project_name = " +
    "'" +
    req.params.name.split("_").join(" ") +
    "' AND email = '" +
    req.params.email +
  ...
  pool.query(query, (error, data) => {
    if (error) {
      ...
      res
        .status(500)
        .send("Internal server error when retrieving backed rewards");
    } else {
      res.status(200).send(data.rows);
    }
  });
...
router.post("/:name/comments", function(req, res, next) {
  const query =
    "INSERT INTO comments (project_name, comment_text, email) VALUES " +
    "('" +
    req.body.projectName +
    "',$$" +
    req.body.newComment +
    "$$,'" +
    req.body.commenterEmail +
    "') RETURNING *";
  pool.query(query, (error, data) => {
    if (error) {
      console.log(error);
...
router.get("/:name/updates", function(req, res, next) {
  const query =
    "SELECT * FROM updates WHERE project_name = " +
    "'" +
    req.params.name.split("_").join(" ") +
    "'";
  pool.query(query, (error, data) => {
...
router.get("/:name/comments", function(req, res, next) {
  const query =
    "SELECT * FROM comments WHERE project_name = " +
    "'" +
    req.params.name.split("_").join(" ") +
...
router.post("/:name/back", function(req, res) {
  ...
  const query = `
  SELECT * FROM backs('${user_email}', '${project_backed_name}', '${reward_name}' , ${backs_amount})
  `;
  pool.query(query, (error, data) => {
    if (error) {
      console.log(error);
